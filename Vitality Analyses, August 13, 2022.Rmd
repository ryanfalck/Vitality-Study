...
lmm.ex.mdl<- function(y,cov1,cov2,baselinemean, baselinese){
  ...
  testframe$within.diff_LL<-testframe$within.diff.means - 1.96*testframe$within.diff.se
  testframe$within.diff_UL<-testframe$within.diff.means + 1.96*testframe$within.diff.se 
  ...
  return(list(summary=modelsummary, lsmeans=means.1, group_contrasts=contrasts, 
              contrasts_ci.95=conf.95, testframe,diff_frame,VIF))
}
```
...
nbb.ex.mdl<- function(z,covar1,covar2,reverse,baselinemean1,baseline1.se){
  ...
  return(list(Means=marginalmeans,Within_Change=within,Between_Change=between,VIF))
}
```
